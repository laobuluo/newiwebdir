...
require(CORE_PATH.'module/comment.php');
...
if (is_file($modpath)) {
	...
	if (preg_match("/^http(s)?:\/\/?([^\/]+)/i", $refurl, $matches)) {
		$domain = $matches[2];
		if (!empty($domain)) {
			$DB->query("UPDATE ".$DB->table('websites')." w, ".$DB->table('webdata')." d SET d.web_instat=d.web_instat+1, d.web_itime=".time()." WHERE w.web_id=d.web_id AND web_url='$domain'");
		}				
	}
} else {
	_404();
