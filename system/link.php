...
if ($action == 'list') {
	...
	$results = get_link_list($where, 'link_id', 'DESC', $start, $pagesize);
	$links = array();
	foreach ($results as $row) {
		...
		$row['link_oper'] = '<a href="'.$fileurl.'?act=edit&link_id='.$row['link_id'].'">编辑</a>&nbsp;|&nbsp;<a href="'.$fileurl.'?act=del&link_id='.$row['link_id'].'" onClick="return confirm(\'确认删除此内容吗？\');">删除</a>';
		$links[] = $row;
	}
...
if ($action == 'edit') {
	...
	$row = get_one_link($link_id);
	if (!$row) {
		alert('指定的内容不存在！');
	}
	
	$smarty->assign('display', $row['link_hide']);
